...
def _find_card_rows(key: str, *, card_level_only: bool = False,
    ...
    if exact:
        ...
        #
        # 更正一個我寫錯的理由：我原本說「判不出重複最多讓一張重複的卡片
        # 過去，下一輪還抓得到」。不會——收錄之後它的網址就進了
        # existing_by_topic，永遠不再是候選。兩種失敗都是永久的，所以那個
        # 論證根本不適用。真正的理由是：拿別張卡的向量去判，錯的是「判斷
        # 本身無效」；查不到而不判，錯的是「沒有判斷」——後者至少不會把一個
        # 無效的判斷寫成永久決策。而查不到這件事本身要出聲，見呼叫端。
        return []
