...
def _is_valid_orphan_card(text: str, card_id: str, source_url: str, title: str, summary: str) -> bool:
    ...
    if source_type == "x-bookmark":
        if not _looks_like_url(source_url) or not _extract_status_id(source_url):
            return False
    elif source_type in {"youtube", "github_star", "github_fork"}:
        if not _looks_like_url(source_url):
            return False
    elif source_type == "local-paper":
        if not (source_url and (source_url.startswith("/") or _looks_like_url(source_url))):
            return False
