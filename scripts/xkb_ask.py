...
try:
    from contrarian_recall import recall as _contrarian_recall
...
- 如果上下文最後有「已知反例或限制」區塊，在回答結尾自然點出，語氣像「不過這裡有個值得注意的地方...」，不要刻意放大也不要忽略
...
def build_answer(query: str, wiki_hits: list[dict], card_hits: list[dict],
    ...
    # Contrarian recall: surface known counter-examples or warnings
    contrarian_block = ""
    if _CONTRARIAN_AVAILABLE and situation in ("enrichment", "continuity"):
        try:
            c_hits = _contrarian_recall(query, top_k=2)
            if c_hits:
                clines = ["\n\n[已知反例或限制] 如有相關請在回答末尾自然提及："]
                for r in c_hits:
                    src = r.source_file.split("/")[-1]
                    clines.append(f"- [{src} | {r.section}] {r.excerpt}")
                contrarian_block = "\n".join(clines)
        except Exception:
            pass
    ...
    prompt = CONTEXT_TMPL.format(
        context="\n\n".join(entries) + links_block + contrarian_block,
        query=query,
    )
