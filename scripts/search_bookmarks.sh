...
if [[ "$AUTO_INDEX_UPDATE" == "1" ]] && [[ -x "/home/admin/.openclaw/workspace/skills/x-knowledge-base/scripts/build_search_index.sh" ]]; then
    /home/admin/.openclaw/workspace/skills/x-knowledge-base/scripts/build_search_index.sh --incremental >/dev/null 2>&1 || true
fi

# 若索引仍不存在，再做一次全量建立
if [[ ! -f "$INDEX_FILE" ]] && [[ -x "/home/admin/.openclaw/workspace/skills/x-knowledge-base/scripts/build_search_index.sh" ]]; then
    /home/admin/.openclaw/workspace/skills/x-knowledge-base/scripts/build_search_index.sh >/dev/null 2>&1 || true
fi
