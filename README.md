...
XKB now has **two Minions-native pipelines** on top of Postgres-backed GBrain:

1. **Bookmark enrichment pipeline** for X/Twitter bookmarks
2. **Memory distill pipeline** for daily memory → wiki staging

The newer memory distill flow uses an **adapter-based three-stage pipeline** so inference runtimes can be swapped without changing XKB core logic.
...
### Adapter-based memory distill pipeline

Flow:

```text
prep worker
  -> cleaned.txt + chunk files + manifest.json
  -> enqueue chunk jobs
...
infer queue / consumer
  -> dispatches request artifact to adapter runtime
  -> writes standardized result artifact

consolidate worker
  -> loads chunk result artifacts
  -> reranks / consolidates insights
  -> writes wiki staging file
```

Key scripts:
- `scripts/distill_memory_minion_worker.py`
- `scripts/distill_memory_chunk_worker.py`
- `scripts/xkb_infer_enqueue.py`
- `scripts/xkb_infer_consumer.py`
- `scripts/distill_memory_consolidate_worker.py`

Adapter interface:
- `scripts/xkb_adapter_base.py`
- `scripts/xkb_adapter_openclaw.py`
- `scripts/xkb_adapter_claude_code.py`
- `scripts/xkb_adapter_minimax.py`

Request/result artifacts are file-based (`xkb.infer.v1` / `xkb.result.v1`), which keeps the XKB core independent from any single inference backend.
...
- adapter-based inference handoff for distill workloads
...
- memory distill prep → chunk request artifact → infer consumer → consolidate → staging has been run end-to-end

### Adapter spec

Any runtime can participate if it can:
- read one request artifact
- produce one result artifact in the normalized schema

That is the contract behind future adapters like Claude Code or direct MiniMax API batch execution.
...
When a user sends a message, XKB classifies the conversation state and routes to the appropriate recall strategy:

```
User message
    │
    ▼
conversation_state_parser.py   ← trigger class (hard/soft/suppress) + state
    │
    ▼
recall_router.py               ← unified routing entry point
    ├── hard trigger  →  continuity_recall (MEMORY.md + wiki) + action_recall
    └── soft trigger  →  wiki recall + associative recall + contrarian_recall
            │
            ▼
    _session_dedup.py          ← filter already-shown (source, section) pairs
            │                     TTL 4h, /tmp session file
            ▼
    Delivery (inline_injection | side_hint | expandable_hint | none)
```

Two knowledge layers are always searched in order:
...
Contrarian recall (`contrarian_recall.py`) supplements with warnings, known failures, and counter-examples, surfaced at the end of enrichment and continuity answers.
...
# Run the recall router directly
python3 scripts/recall_router.py "XKB 下一步是什麼" --json
python3 scripts/recall_router.py "query" --dry-run
```
...
| Script | What it does |
|--------|-------------|
| `recall_router.py` | Unified routing entry point: classifies trigger, routes to recall strategies, deduplicates |
| `conversation_state_parser.py` | Classifies user message into trigger class (hard/soft/suppress) + conversation state |
| `xkb_ask.py` | Natural-language Q&A: wiki (Layer 1) → cards via XBrain hybrid search (Layer 2) |
| `recall_for_conversation.py` | Conversation-triggered recall (wiki + XBrain card search + multi-layer sources) |
| `continuity_recall.py` | MEMORY.md + wiki lookup for session continuity |
| `contrarian_recall.py` | Surfaces warnings, failures, counter-examples from knowledge base |
| `action_recall.py` | Action-oriented recall (what to do next) |
| `_session_dedup.py` | TTL-based session deduplication — prevents same (source, section) pair from being pushed twice in one session |
...
| v1.1 | ✅ | **Active Recall quality upgrade** — `recall_router.py` unified routing (hard/soft/suppress trigger classes); contrarian recall integrated into enrichment flow; session deduplication (TTL 4h) prevents repeat pushes |
